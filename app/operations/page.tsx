...
export default async function Operations({ searchParams }: { searchParams: SearchParams }) {
  ...
  const activeTab = tabs.some((item) => item.id === tab) ? tab : "queue";
  ...
  const [{ data: posts }, { data: metrics }, { data: runs }] = await Promise.all([
    ...
    db.from("metrics").select("reach, saves, shares, likes, comments, profile_visits, follows, captured_at").order("captured_at", { ascending: false }).limit(100),
    db.from("runs").select("id, job, ok, started_at, finished_at, error").order("started_at", { ascending: false }).limit(20),
  ]);
      ...
      <nav aria-label="Operations sections" style={{ display: "flex", gap: 8, marginBottom: 22, overflowX: "auto" }}>
        {tabs.map((item) => (
          <Link key={item.id} href={"/operations?tab=" + item.id} style={{
            ...
            textDecoration: "none", fontFamily: "var(--data)", fontSize: 12, letterSpacing: ".08em", textTransform: "uppercase", borderRadius: 3,
          }}>{item.label}</Link>
        ))}
      </nav>
      ...
      {activeTab === "queue" && (
        <section style={panel}>
          <div style={{ padding: 20, borderBottom: "1px solid var(--edge)", display: "flex", justifyContent: "space-between", gap: 16 }}>
            <div><h2 style={{ fontSize: 18 }}>Publishing queue</h2><p style={{ ...mono, marginTop: 6 }}>Every scheduled item, from plan through publish.</p></div>
          ...
          <div>
            ...
            {!posts?.length && <p style={{ padding: 28, ...mono }}>The queue is empty. The planner will refill it on its next run.</p>}
          </div>
        </section>
      )}
