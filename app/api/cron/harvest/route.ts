...
/**
 * Publishing + metrics ingestion.
 * Gated on igConfigured — with IG env vars unset (current state), this cron
 * runs, does nothing, and reports ig_configured: false. That is expected.
 */
export async function GET(req: Request) {
  ...
  return Response.json(await withRun("harvest", async () => {
    ...
    return { published, harvested, ig_configured: igConfigured };
  }));
}
